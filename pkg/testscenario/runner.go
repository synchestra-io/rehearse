...
func execScript(language, script string, env []string) (stdout, stderr string, exitCode int, err error) {
	...
	case "sql":
		// SQL scripts are executed via DATABASE_URL env var or default to sqlite3.
		// The script is passed via stdin to the database CLI.
		dbURL := ""
		for _, e := range env {
			if k, v, ok := strings.Cut(e, "="); ok && k == "DATABASE_URL" {
				dbURL = v
			}
		}
		if dbURL != "" && strings.HasPrefix(dbURL, "postgres") {
			cmd = exec.Command("psql", dbURL, "-c", script)
		} else {
			// Default to sqlite3 with the DATABASE_URL as the file path, or in-memory.
			dbPath := dbURL
			if dbPath == "" {
				dbPath = ":memory:"
			}
