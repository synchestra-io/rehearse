...
This package has no external dependencies beyond the Go standard library and a
markdown parser. It receives a configurable spec root path and resolves AC
references from the filesystem.

## Spec

See [`spec/features/testing-framework/`](../../spec/features/testing-framework/README.md)
...
```bash
go run . run spec/features/testing-framework/test-runner/_tests/runner-core.md --format json
```

See the
[Test Runner feature spec](../../spec/features/testing-framework/test-runner/README.md#dogfooding)
...
go run . run spec/features/testing-framework/test-runner/_tests/progress-demo.md

# Step failure — shows how errors are reported live
go run . run spec/features/testing-framework/test-runner/_tests/error-demo.md
```

To include manual scenarios when running a directory:

```bash
go run . run spec/features/testing-framework/test-runner/_tests/ --run-manual-tests
```
